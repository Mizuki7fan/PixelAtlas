...
MKLPardisoSolver::~MKLPardisoSolver() {
  ...
  pardiso(pt, &maxfct, &mnum, &mtype, &phase, &num, &ddum, ia.data(), ja.data(),
          &idum, &nrhs, iparm, &msglvl, &ddum, &ddum, &error);
}

void MKLPardisoSolver::PardisoInit() {
  mtype = -2;
  if (mtype == -1)
  ...
  phase = 11;
  // cout << "err" << endl;
  pardiso(pt, &maxfct, &mnum, &mtype, &phase, &num, a.data(), ia.data(),
...
void MKLPardisoSolver::PardisoSolver() {
  ...
  pardiso_printstats(&mtype, &numRows, a.data(), ia.data(), ja.data(), &nrhs,
                     rhs.data(), &error);
  if (error != 0) {
  ...
  pardiso(pt, &maxfct, &mnum, &mtype, &phase, &num, a.data(), ia.data(),
          ja.data(), &idum, &nrhs, iparm, &msglvl, rhs.data(), result.data(),
          &error);
...
void MKLPardisoSolver::FreeNumericalFactorizationMemory() {
