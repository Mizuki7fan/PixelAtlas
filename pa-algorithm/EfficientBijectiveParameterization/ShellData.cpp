...
void ShellData::mesh_improve() {
  ...
  Eigen::MatrixXd H = Eigen::MatrixXd::Zero(component_sizes.size(), 2);
  {
    int hole_f = 0;
    int hole_i = 0;
...
void ShellData::add_new_patch(const Eigen::MatrixXd &V_in,
  ...
  M.resize(F_ref.rows());
  {
    ...
    double area_f;
    for (size_t i = 0; i < static_cast<std::size_t>(F_ref.rows()); i++) {
  ...
  for (auto cur_bnd : all_bnds) {
    ...
    internal_bnd.bottomRows(cur_bnd.size()) =
        Eigen::Map<Eigen::ArrayXi>(cur_bnd.data(), cur_bnd.size()) + mv_num;
