...
void Parafun::init() {
  ...
  solver->ia_ = pardiso_ia;
  solver->ja_ = pardiso_ja;
  solver->a_.resize(pardiso_ja.size());
  solver->nnz_ = pardiso_ja.size();
  solver->num_ = 2 * V_N;
...
void Parafun::setvirtualtri() {
  Eigen::VectorXi boundary_vertex = d_.frame_ids;
...
void Parafun::Pre_calculate() {
  ...
  VV_tmp.resize(V_N);
  for (std::size_t i = 0; i < static_cast<std::size_t>(d_.m_T.rows()); i++) {
    int vid[3];
    for (std::size_t j = 0; j < static_cast<std::size_t>(d_.m_T.cols()); j++) {
      vid[j] = d_.m_T(i, j);
    }
  ...
  for (std::size_t i = 0; i < static_cast<std::size_t>(d_.s_T.rows()); i++) {
    s_vid.clear();
    for (std::size_t j = 0; j < static_cast<std::size_t>(d_.s_T.cols()); j++) {
      int s_id = d_.s_T(i, j);
...
void Parafun::handle_mintri() {
  ...
  int acc_bnd = 0;
  for (std::size_t i = 0; i < d_.bnd_sizes.size(); i++) {
...
double Parafun::BPE(bool is_ip_convrate, bool is_slim_convrate) {
  ...
  d_.w_uv = Eigen::Map<Eigen::Matrix<double, -1, -1, Eigen::ColMajor>>(
...
void Parafun::SLIM(bool is_interp) {
  ...
  std::vector<double> result_d = solver->result_;
...
void Parafun::max_step(const Eigen::VectorXd &xx, const Eigen::VectorXd &qq,
  ...
  double b2, x0_up, x1_up, x2_up, y0_up, y1_up, y2_up;
  ...
  do {
    ...
    check_isintersection = check_intersection(xx + temp_t * qq);
...
void Parafun::local_coordinate_inverse(int i, double &p00, double &p01,
  ...
  if (area_tri > area_min) {
    Eigen::Vector3d t_(d_.m_V(f0, 0), d_.m_V(f0, 1), d_.m_V(f0, 2));
    Eigen::Vector3d u_(d_.m_V(f1, 0), d_.m_V(f1, 1), d_.m_V(f1, 2));
    Eigen::Vector3d v_(d_.m_V(f2, 0), d_.m_V(f2, 1), d_.m_V(f2, 2));
    Eigen::Vector3d x_(d_.m_V(f1, 0) - d_.m_V(f0, 0),
                       d_.m_V(f1, 1) - d_.m_V(f0, 1),
                       d_.m_V(f1, 2) - d_.m_V(f0, 2));
    double x1_0 = x_.norm();
    x_ /= x1_0;
    Eigen::Vector3d l_(d_.m_V(f2, 0) - d_.m_V(f0, 0),
                       d_.m_V(f2, 1) - d_.m_V(f0, 1),
...
void Parafun::local_coordinate_inverse_scaf(int i, double &p00, double &p01,
  ...
  Eigen::Vector2d x_(d_.w_uv(f1, 0) - d_.w_uv(f0, 0),
                     d_.w_uv(f1, 1) - d_.w_uv(f0, 1));
  Eigen::Vector2d l_(d_.w_uv(f2, 0) - d_.w_uv(f0, 0),
                     d_.w_uv(f2, 1) - d_.w_uv(f0, 1));
